...
function LanguageSwitch({ handleLanguageSwitch, handleFileNameInputChange, currentLanguage }) {
    ...
    const initialSelected = buttons.find(btn => btn.id === currentLanguage.language)
    const [selected, setSelected] = useState(initialSelected);
    const [extension, setExtension] = useState(currentLanguage.extension);
    const [fileName, setFileName] = useState(currentLanguage.file_name);

    useEffect(() => {
        setSelected(buttons.find(btn => btn.id === currentLanguage.language));
        setExtension(currentLanguage.extension);
        setFileName(currentLanguage.file_name);
    }, [currentLanguage]);
            ...
            <div className=' px-2 relative'>
                <p>
                    <input
                        ...
                        onBlur={() => handleFileNameInputChange(fileName)}
                    >
                    </input>
